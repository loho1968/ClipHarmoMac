...
| 短信验证码 | 手机复制验证码 → Mac 写入剪贴板 + 通知（鸿蒙权限限制下用 App「手动发送」触发） |
...
**Q: 手机上复制文字，Mac 为什么不会自动收到？**

A: HarmonyOS API 12 起对剪贴板读取做了权限管控：程序化读取剪贴板（getData 等）需要受限权限 `ohos.permission.READ_PASTEBOARD`（需在 AGC 申请并随签名 profile 下发）。未获授权时读取会静默返回空，因此手机端"复制即自动推送"在当前系统上不可用（这是平台限制，非两端 bug）。

替代用法：在手机 App 的"手动发送"输入框中输入/粘贴内容后点发送，Mac 即会收到（该入口不依赖剪贴板读取权限）。Mac→手机方向不受影响（写入剪贴板无需该权限）。
