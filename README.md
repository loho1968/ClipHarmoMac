...
| 短信验证码 | 手机复制验证码 → Mac 自动写入剪贴板 + 通知（需授予 READ_PASTEBOARD 权限） |
| 局域网自动发现 | 同一 WiFi 下 UDP 广播发现，零手动操作 |
| 云中继 | 不同网络（5G ↔ WiFi）通过 WebSocket 中继同步 |
| 二维码配对 | 扫码完成配对，同步设置中继地址和局域网 IP |
| 端到端加密（⚠️ 已禁用） | 曾用 ECDH P-256 + AES-256-GCM；2026-09 起两端全局明文（密钥不同步致乱码，d45ee2a），恢复待决策 |
...
A: 当前**不加密（明文）**。端到端加密（ECDH P-256 → AES-256-GCM）已于 2026-09 全局禁用（提交 d45ee2a）：此前两端 HKDF 派生密钥不同步导致乱码。加密代码保留但开关恒为 false，LAN 与中继均为明文；中继链路也尚未启用 TLS（明文 ws://）。恢复加密需先解决两端密钥同步问题。
...
**Q: 手机上复制文字，Mac 会自动收到吗？**

A: 会。手机端“复制即自动推送”需要剪贴板读取权限 `ohos.permission.READ_PASTEBOARD`（HarmonyOS API 12+ 对程序化读取剪贴板做了管控，属受限 user_grant 权限）。本项目已在代码中声明并在启动时申请一次，但生效前提是：① 在 AGC 申请该权限并审批通过；② 签名 Profile 重新生成后其 ACL 含该权限（`./signing/ClipboardSyncDebug.p7b`）；③ 首次启动时弹窗点“允许”。授权后，在其它 App 复制文字，ClipboardSync 即可自动推送 Mac 写入剪贴板。

若未授权（或权限被拒），可用 App 内“手动发送”输入框发送文本到 Mac（该入口不依赖剪贴板读取权限）。Mac→手机方向不受影响（写入剪贴板无需该权限）。
