...
| 功能 | 状态 | 备注 |
|------|------|------|
| Mac → 鸿蒙 文本同步 | ✅ 已通 | Mac 复制文字，鸿蒙端自动写入剪贴板 |
| 鸿蒙 → Mac 文本同步 | ✅ 已通 | 鸿蒙复制文字，Mac 端自动写入剪贴板 |
| UDP 自动发现设备 | ⚠️ 部分工作 | Mac 端能收到鸿蒙广播；鸿蒙端收到 Mac 广播后自动连接还需调试 |
| 手动输入 IP 连接 | ✅ 已通 | 鸿蒙端输入 Mac IP 后连接成功 |
| 图片剪贴板同步 | ⏳ 框架已有 | Mac 端已支持读取/发送图片(PNG Base64)，鸿蒙端接收尚未实现 |
| 去重防回环 | ✅ 已通 | 通过 timestamp 判断，避免写入→监听→回传死循环 |
| 同步历史记录 | ✅ 已通 | 两端 UI 显示最近 50 条同步记录 |
...
- [ ] **UDP 自动发现连接**：鸿蒙端 DiscoveryService 收到 Mac 广播后需正确提取 `remoteInfo.address`，自动发起 TCP 连接，免去手动输入 IP

### P1 - 体验优化

- [ ] **图片剪贴板同步**：鸿蒙端实现接收图片消息并写入系统剪贴板
- [ ] **Mac 端菜单栏状态图标**：已连接/未连接/搜索中显示不同图标颜色
- [ ] **Mac 端开机自启**：注册为 LaunchAgent
- [ ] **鸿蒙端后台保活**：申请 ContinuousTask（`backgroundModes: dataTransfer`），保持后台同步
- [ ] **连接状态持久化**：记住上次连接的 Mac IP，下次自动连接

### P2 - 安全与扩展

- [ ] **端到端加密**：两端配对时交换密钥，AES-256-GCM 加密传输内容
- [ ] **跨 WiFi/广域网支持**：部署中继服务器，两端通过 WebSocket 连接中继
- [ ] **多设备支持**：Mac 端接受多个 TCP 连接，支持多台鸿蒙设备
- [ ] **大文件传输优化**：图片/大文本使用分片传输，避免单条消息过大
